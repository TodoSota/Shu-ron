...
mpmObject::mpmObject(GLsizei count, int gridSize) :
	...
	count{ count },
	// テクスチャを作成
	gridTex{ []() {GLuint t; glGenTextures(1, &t); return t; }() },
	// グリッドサイズを保存
	gridSize{ gridSize }
{
	// バッファの設定
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(Particle) * count, nullptr, GL_DYNAMIC_DRAW);// vboのメモリを確保し頂点位置データを転送
	...
	glVertexAttribIPointer(1, 1, GL_INT, sizeof(Particle), (void*)offsetof(Particle, state)); // int なので 1 成分
	glEnableVertexAttribArray(1); // 1 番のvboを有効に

	// 3Dテクスチャ(MPMグリッド)の設定
	// RGBA32F を使用: RGB =運動量(mvx, mvy, mvz), A=質量(m)
	glBindTexture(GL_TEXTURE_3D, gridTex);
	glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA32F, gridSize, gridSize, gridSize, 0, GL_RGBA, GL_FLOAT, nullptr);
...
mpmObject::~mpmObject() {
	...
	glDeleteTextures(1, &gridTex);	//3Dテクスチャを削除
}