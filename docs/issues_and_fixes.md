...
## 5. 既知の課題（Unresolved Issues）

現在、機能拡張を優先して開発を進めているため、根本的な解決に至っていない既知の課題を以下に記録する。これらは将来的なアップデートでの修正を予定している。

### 5.1 エネルギー計算式の見直しに伴うスロッシング現象

### ■ 問題

より正確な物理挙動を目指してエネルギー計算（応力・ひずみの更新式など）を見直した結果、シミュレーション領域の境界（端）付近において、粒子が沸騰したように揺れ動く現象（スロッシング）が発生するようになった。

※ 該当の挙動 : 領域端での沸騰現象
![debu5](../media/debug5.gif)


### ■ 原因推測と現状の判断

エネルギー保存則が厳密に働きすぎているため、境界での運動エネルギーが適切に減衰せずに反射を繰り返している、または後述するSDF境界でのペナルティ押し出し力との相互作用が原因と推測している。
現在は、全体のリアリティ向上と新規機能の実装を優先し、意図的に対応を見送っている。

### 5.2 SDF特有のめり込みとエッジ判定の不安定さ

### ■ 問題

任意形状のオブジェクトをSDFとして導入した結果、高速で移動する粒子がオブジェクトにめり込む、あるいは鋭利なエッジ（角）部分において不自然な反発挙動を示すケースが確認されている。


### ■ 原因推測と現状の判断

* **めり込み**: 現在のペナルティ法（めり込んだ深さに応じて押し出す力）を用いた計算では、1フレーム間で大きくめり込んだ場合に対応しきれていない（連続衝突判定の不在）。
* **エッジ判定**: 素朴な総当たりで生成した解像度 `64^3` のSDFテクスチャでは、鋭角なエッジの勾配（法線）表現に限界があり、補間誤差が生じていると考えられる。
こちらも、より高度な機能実装を優先しているため、当面の仕様として許容している。
